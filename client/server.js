...
function server(listenPort, consortiumId, storageAccount, storageAccountKey, containerName, identityBlobPrefix, ethRpcPort, validatorListBlobName,
...
var corsOptions = {
  origin: function (origin, callback) {
    if (origin.toLowerCase().indexOf("azure.com") !== -1) {
      callback(null, true)
    } else {
...
app.get('/getNodes', cors(corsOptions), function (req, res) {
...
// Get:networkinfo
// We allow any origin for this endpoint since it must be reached from joining members
app.get('/networkinfo', cors(), function (req, res) {
...
app.get('/selectValidators/:maxValidators', cors(corsOptions), function (req, res) {
  getBlobs()
  .then(getLeasedBlobList)
  .then(getActiveNodeDetails).catch(function (error) {
